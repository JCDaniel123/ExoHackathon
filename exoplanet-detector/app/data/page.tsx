...
import { Upload, Download, Settings, ArrowUpDown, Search } from "lucide-react"
          ...
          <CardHeader>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                ...
                <CardDescription>{filteredData.length} exoplanets loaded</CardDescription>
              </div>
                ...
                <TableHeader>
                  <TableRow>
                    ...
                    {visibleColumns.disposition_score && (
                      <TableHead>
                        <Button variant="ghost" onClick={() => handleSort("disposition_score")} className="h-8 px-2">
                    ...
                    {visibleColumns.planetary_radius && (
                      <TableHead>
                        <Button variant="ghost" onClick={() => handleSort("planetary_radius")} className="h-8 px-2">
                    ...
                    {visibleColumns.insolation_flux && (
                      <TableHead>
                        <Button variant="ghost" onClick={() => handleSort("insolation_flux")} className="h-8 px-2">
                    ...
                    {visibleColumns.surface_gravity && (
                      <TableHead>
                        <Button variant="ghost" onClick={() => handleSort("surface_gravity")} className="h-8 px-2">
                    ...
                    {visibleColumns.transit_duration && (
                      <TableHead>
                        <Button variant="ghost" onClick={() => handleSort("transit_duration")} className="h-8 px-2">
                    ...
                    {visibleColumns.impact_parameter && (
                      <TableHead>
                        <Button variant="ghost" onClick={() => handleSort("impact_parameter")} className="h-8 px-2">
                    ...
                    {visibleColumns.stellar_magnitude && (
                      <TableHead>
                        <Button variant="ghost" onClick={() => handleSort("stellar_magnitude")} className="h-8 px-2">
                  ...
                  {loading ? (
                    ...
                    filteredData.map((row) => (
                      <TableRow key={row.id}>
                        ...
                        {visibleColumns.disposition_score && <TableCell>{row.disposition_score.toFixed(3)}</TableCell>}
                        {visibleColumns.planetary_radius && <TableCell>{row.planetary_radius.toFixed(2)}</TableCell>}
                        {visibleColumns.insolation_flux && <TableCell>{row.insolation_flux.toFixed(2)}</TableCell>}
                        {visibleColumns.surface_gravity && <TableCell>{row.surface_gravity.toFixed(2)}</TableCell>}
                        {visibleColumns.orbital_period && <TableCell>{row.orbital_period.toFixed(2)}</TableCell>}
                        {visibleColumns.transit_duration && <TableCell>{row.transit_duration.toFixed(2)}</TableCell>}
                        {visibleColumns.transit_depth && <TableCell>{row.transit_depth.toFixed(4)}</TableCell>}
                        {visibleColumns.impact_parameter && <TableCell>{row.impact_parameter.toFixed(3)}</TableCell>}
                        {visibleColumns.stellar_magnitude && <TableCell>{row.stellar_magnitude.toFixed(2)}</TableCell>}
                        {visibleColumns.classification && (
                          <TableCell>
                            <Badge
                              variant={
